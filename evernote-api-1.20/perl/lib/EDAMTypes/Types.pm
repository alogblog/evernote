...
use base qw(Class::Accessor);
EDAMTypes::NoteAttributes->mk_accessors( qw( subjectDate latitude longitude altitude author source sourceURL sourceApplication shareDate taskDate taskCompleteDate taskDueDate placeName contentClass applicationData ) );
...
sub new {
  ...
  $self->{taskDate} = undef;
  $self->{taskCompleteDate} = undef;
  $self->{taskDueDate} = undef;
  ...
  if (UNIVERSAL::isa($vals,'HASH')) {
    ...
    if (defined $vals->{taskDate}) {
      $self->{taskDate} = $vals->{taskDate};
    }
    if (defined $vals->{taskCompleteDate}) {
      $self->{taskCompleteDate} = $vals->{taskCompleteDate};
    }
    if (defined $vals->{taskDueDate}) {
      $self->{taskDueDate} = $vals->{taskDueDate};
    }
...
sub read {
  ...
  while (1) 
  {
    ...
    SWITCH: for($fid)
    {
      ...
      last; };
      /^18$/ && do{      if ($ftype == TType::I64) {
        $xfer += $input->readI64(\$self->{taskDate});
      } else {
        $xfer += $input->skip($ftype);
      }
      last; };
      /^19$/ && do{      if ($ftype == TType::I64) {
        $xfer += $input->readI64(\$self->{taskCompleteDate});
      } else {
        $xfer += $input->skip($ftype);
      }
      last; };
      /^20$/ && do{      if ($ftype == TType::I64) {
        $xfer += $input->readI64(\$self->{taskDueDate});
      } else {
        $xfer += $input->skip($ftype);
      }
      last; };
...
sub write {
  ...
  if (defined $self->{taskDate}) {
    $xfer += $output->writeFieldBegin('taskDate', TType::I64, 18);
    $xfer += $output->writeI64($self->{taskDate});
    $xfer += $output->writeFieldEnd();
  }
  if (defined $self->{taskCompleteDate}) {
    $xfer += $output->writeFieldBegin('taskCompleteDate', TType::I64, 19);
    $xfer += $output->writeI64($self->{taskCompleteDate});
    $xfer += $output->writeFieldEnd();
  }
  if (defined $self->{taskDueDate}) {
    $xfer += $output->writeFieldBegin('taskDueDate', TType::I64, 20);
    $xfer += $output->writeI64($self->{taskDueDate});
    $xfer += $output->writeFieldEnd();
  }
...
sub write {
  ...
  if (defined $self->{sharedNotebookIds}) {
    $xfer += $output->writeFieldBegin('sharedNotebookIds', TType::LIST, 13);
    {
      ...
      {
        foreach my $iter57 (@{$self->{sharedNotebookIds}}) 
        {
